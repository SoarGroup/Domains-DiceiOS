...
- (DiceSMLData *)GameStateToWM:(DiceGameState *)game withInputLink:(sml::Identifier *)inputLink andMyPlayerIDIs:(int) playerID
{
    ...
    NSLog(@"State id name: %s", idState->GetIdentifierName());
    
    idPlayers = inputLink->CreateIdWME("players");
    idAffordances = inputLink->CreateIdWME("affordances");
    
    PlayerState *selfPlayer = [game getPlayerWithID:playerID];
    ...
    if (playerHasWon)
        victorID = [[game gameWinner] getID];
    ...
    for (PlayerState *player in [game players])
    {
        if ([player isKindOfClass:[PlayerState class]])
        {
            ...
            playerId->CreateStringWME("exists", ([player hasLost] ? "true" : "false"));
            Identifier *cup = playerId->CreateIdWME("cup");
            NSLog(@"Cup wme id: %s", cup->GetIdentifierName());
            if (strlen(cup->GetIdentifierName()) < 2)
            {
                NSLog(@"too short!");
            }
...
- (BOOL) handleAgentCommandsWithGameState:(DiceGameState *)gameState andPlayerID:(int)playerID withNeedsRefreshBool:(BOOL *)needsRefresh withInformation:(turnInformationSentFromTheClient *)info withErrors:(int)errors
{
    ...
    for (int j = 0; j < agent->GetNumberCommands(); j++)
    {
        ...
        if (![attrName isEqualToString:@"qna-query"])
        {
            ...
            {
                Bid *bid = [[[Bid alloc] initWithPlayerID:playerID
                                                     name:self.name
                                                     dice:[[NSString stringWithUTF8String:ident->GetParameterValue("multiplier")] intValue]
                                                     rank:[[NSString stringWithUTF8String:ident->GetParameterValue("face")] intValue]]
                            autorelease];
            ...
            else if ([attrName isEqualToString:@"push"])
            {
                ...
                if (goodCommand)
                {
                    ...
                    NSMutableArray *muteArray = [[[NSMutableArray alloc] init] autorelease];
                    ...
                    NSArray *staticArray = [[[NSArray alloc] initWithArray:muteArray] autorelease];
            ...
            else if ([attrName isEqualToString:@"challenge"])
            {
                ...
                if (target != -1)
                {
                    ...
                    {
                        if ([[gameState lastHistoryItem] actionType] == A_PASS)
